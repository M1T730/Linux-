# PCI passthrough(GPU) and USB passthough (attept) 
...
**objective:** 
The objective was to create a PC running Proxmox, but with a Special VM, running windows, to game on, to have both a node for my cluster and a gaming setup, for a vm to run games, it could not just use the CPU provided by the hypervisor, It needed to be passedthough GPU, usb ports, audio ... \
As Stated in the "result" the usb ports and audio was pretty straightfoward and simple, while the gpu passthough failed. 
I would like to mention, that the VM was not my request, but my little brother wanted it to keep playing his games from the Laptop. 

LAST EDIT : 2/08/2026 (added objective)